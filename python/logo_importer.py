...
class LogoExporter:
    ...
    def run(self):
        import_dir_path = LocalConfigs.repository_directory().joinpath("image\\logo-import")
        if not Helper.verify_exist_directory(import_dir_path):
        ...
        for src_png_name in os.listdir(import_dir_path):
            ...
            if src_png.height > LogoExporter.HEIGHT:
                ...
                if dst_png.width < (dst_png.height * 2):
                    ...
                    new_png.paste(dst_png, (left, 0), mask=dst_png)
                    dst_png = new_png
            dst_png_path = LocalConfigs.repository_directory().joinpath("image", src_png_name)
