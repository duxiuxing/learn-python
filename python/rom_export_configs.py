...
from python.roms_xml import RomsXML
...
class RomExportConfigs:
    ...
    def parse(self, xml_file_name="rom-export.xml"):
        ...
        if "dst_folder" in root.attrib:
            self.dst_roms_directory = os.path.join(
                LocalConfigs.export_to_directory(), root.get("dst_folder")
            )
        ...
        for rom_elem in root:
            ...
            src_path = RomsXML.compute_rom_path(rom_info)
            ...
            rom_name = rom_title + ConsoleConfigs.rom_file_extension()
...
if __name__ == "__main__":
    ...
    for rom_export_info in rom_export_configs.rom_export_info_list():
        print(
            f"{index}.\t | {rom_export_info.rom_title}{ConsoleConfigs.rom_file_extension()}\t\t | {rom_export_info.game_name}"
        )
        index = index + 1
