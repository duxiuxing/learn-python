...
if __name__ == "__main__":
    ...
    cps2_ra_app_configs = Wii_AppConfigs(
        app_name="Capcom - CP System II",
        base_app_folder_name="cps2",
        rom=None,
    )
    cps2_ra_app_configs.long_description = (
    ...
    cps2_ss_app_configs = Wii_AppConfigs(
        app_name="RA-SS CPS-2",
        base_app_folder_name="cps2",
        rom=None,
    )
    cps2_ss_app_configs.long_description = (
    ...
    while True:
        ...
        try:
            ...
            if number == 1:
                cps2_ra_app_configs.init_playlist_configs()
                RA_Playlist(cps2_ra_app_configs.playlist_configs).export_rom_files()
            elif number == 2:
                device = Wii_AppConfigs.DEVICE_SD

                cps2_ra_app_configs.device = device
                cps2_ra_app_configs.init_playlist_configs()
                cps2_ra_app_configs.playlist_configs.boxarts_folder = None
                cps2_ra_app_configs.playlist_configs.logos_folder = None
                cps2_ra_app_configs.playlist_configs.snaps_folder = None
                cps2_ra_app_configs.playlist_configs.titles_folder = None
                cps2_ra_app_configs.use_favorites_as_playlist = False
                WiiRA_App(cps2_ra_app_configs).export_all()
                ...
                cps2_ss_app_configs.device = device
                WiiSS_App(cps2_ss_app_configs).export_all()
