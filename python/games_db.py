...
class GamesDB:
    ...
    @staticmethod
    def new_game_id():
        game_db = GamesDB._instance()
        if game_db._new_id == 0:
            game_db._new_id = len(game_db._id_to_game)
        else:
            game_db._new_id += 1
        return game_db._new_id
    ...
    @staticmethod
    def query_game(
        game_id: str | None = None, game_title: str | None = None
    ) -> Game | None:
